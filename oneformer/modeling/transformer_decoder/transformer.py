...
class ConditionalAttentionGate(nn.Module):
    ...
    def forward(self, x):
        # Apply the gate to each token's representation
        gate_weights = self.gate(x)
        return gate_weights



class TransformerEncoderLayerWithCAG(TransformerEncoderLayer):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu", normalize_before=False):
        super().__init__(d_model, nhead, dim_feedforward, dropout, activation, normalize_before)
        self.cag = ConditionalAttentionGate(d_model)
    ...
    def forward_post(self, src, src_mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None, pos: Optional[Tensor] = None):
        ...
        gate_weights = self.cag(src)  # Calculate conditional attention weights
        src2 = self.self_attn(
            q, k, value=src, attn_mask=src_mask, key_padding_mask=src_key_padding_mask
        )[0] * gate_weights  # Apply gate to the attention result
...
class TransformerDecoderLayerWithCAG(TransformerDecoderLayer):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu", normalize_before=False):
        super().__init__(d_model, nhead, dim_feedforward, dropout, activation, normalize_before)
        self.cag = ConditionalAttentionGate(d_model)
    ...
    def forward_post(self, tgt, memory, tgt_mask: Optional[Tensor] = None, memory_mask: Optional[Tensor] = None, tgt_key_padding_mask: Optional[Tensor] = None, memory_key_padding_mask: Optional[Tensor] = None, pos: Optional[Tensor] = None, query_pos: Optional[Tensor] = None):
        ...
        # Apply attention gate to the decoder's self-attention
        gate_weights = self.cag(tgt)  # Calculate conditional attention weights
        tgt2 = self.self_attn(
            q, k, value=tgt, attn_mask=tgt_mask, key_padding_mask=tgt_key_padding_mask
        )[0] * gate_weights  # Apply gate to the attention result
